...
def plot_items(df,boxplot_to_show):
    x = df['TempMed']
    y = df['Consumption']
...
def main():
    """
    Import data and output some stats
    """
    ...
    boxplot_to_show = {"Week" :weekconsumption.to_list(), 
                    "Week-end" : WeekendConsumption.to_list(),
                    "rain" : rainconsumption.to_list(), 
                    "No rain" : norainconsumption.to_list()}
    plot_items(df, boxplot_to_show)
